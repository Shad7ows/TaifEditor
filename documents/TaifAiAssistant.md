...
| `propose_file_patch` | Creates a staged side-by-side review for one or more small, anchored line edits after path, file size, text, and SHA-256 snapshot checks succeed. | Explicit Accept is always required; whole-file replacement, outside root, missing/binary/oversized file, stale anchor, overlapping/out-of-range edits, repeated request, or any matching unsaved open editor fail closed. |
...
For an existing file, the model must send `edits`, not a replacement `content` document. Each edit carries one-based inclusive `start_line` and `end_line`, exact `expected_text` from that range, and `replacement`. The deterministic patch engine requires ordered, non-overlapping edits, bounds their count and total affected lines, verifies every expected anchor before creating a proposal, rejects an edit set that replaces the entire file, and preserves the original LF/CRLF and final-newline convention. This lets the agent modify only the intended method, statement, or declaration while preserving all untouched code.

TaifEditor tracks **all** open modified editor files, not only the active tab. When the first anchored edit has a complete target path, line range, and exact expected source anchor, a central VS Code–style split review opens. Both read-only LTR source panes start from the same unchanged source snapshot; while the model streams its replacement, only that validated target range changes in the proposed pane. For a multi-edit call, the first safely validated range is the sole live region; all regions appear in the final review after the complete call validates. While content is still streaming, the workspace identifies itself as a live preview and both Accept and Reject are disabled; it cannot write a file. Once the tool call finishes parsing, the complete final proposal and diff are prepared once and the same surface becomes the final staged review with Arabic RTL controls. Transport completion, final review presentation, and any next autonomous turn are queued as separate event-loop phases so a completed model step does not block editor repaint or input. The file remains unchanged until **قبول التعديل** is selected. Rejection never writes the file and gives the model a concise tool result so it can adapt. Multiple proposed patches are reviewed in deterministic tool-call order; continuation pauses until each staged review is resolved.

On acceptance, TaifEditor repeats root, text/binary/size, SHA-256 snapshot, and all-open-unsaved-editor checks before materializing the validated anchored edits and using `QSaveFile` in binary mode for an atomic commit. A stale source or unsaved matching editor therefore still fails closed even after a proposal is visible. When a safe mutation succeeds, the main window refreshes Project Explorer and Git state, refreshes diagnostics and breadcrumbs, and reloads only a matching editor that is still clean. An editor with unsaved content is never silently reloaded or replaced.
...
| Filesystem safety | Root containment, deterministic anchored line-edit application, whole-file/stale/overlapping/out-of-range edit rejection, staged patch-without-write behavior, acceptance-only atomic save, no-overwrite create, all-open-unsaved-editor protection, and mutation refresh behavior are covered. |
